...
LAST_LINE=$(wc -l < src/main/java/com/allyticlabs/backend/security/TokenGenerator.java)
sed -i "${LAST_LINE}i\\
\\
    /**\\
...
" src/main/java/com/allyticlabs/backend/security/TokenGenerator.java

# Patch 2: Add URL getters to MpesaConfig
echo "2. Patching MpesaConfig with URL getters..."
LAST_LINE=$(wc -l < src/main/java/com/allyticlabs/backend/config/MpesaConfig.java)
sed -i "${LAST_LINE}i\\
\\
...
" src/main/java/com/allyticlabs/backend/config/MpesaConfig.java
...
if ! grep -q "import com.stripe.net.Webhook" src/main/java/com/allyticlabs/backend/service/StripeService.java; then
    sed -i '/^import com.stripe/a\import com.stripe.net.Webhook;' src/main/java/com/allyticlabs/backend/service/StripeService.java
fi

# Patch 4: Add missing methods to PaymentService
echo "4. Patching PaymentService with missing methods..."
if ! grep -q "generatePaymentId()" src/main/java/com/allyticlabs/backend/service/PaymentService.java; then
    # Add imports first
    sed -i '/^import/a\import java.security.MessageDigest;\nimport java.nio.charset.StandardCharsets;\nimport java.util.UUID;\nimport java.time.Instant;\nimport java.math.BigDecimal;' src/main/java/com/allyticlabs/backend/service/PaymentService.java | head -1
    
    # Add methods before the last closing brace
    LAST_LINE=$(wc -l < src/main/java/com/allyticlabs/backend/service/PaymentService.java)
    sed -i "${LAST_LINE}i\\
\\
